...
#hepia.titled_paragraph(
  ...
  [
    ...
    - Concevoir et développer la logique logicielle tirant parti de Serf et K3s pour créer le cluster initial. Lors de l'intégration d'un nouveau nœud, K3s devra s'installer et se configurer de manière autonome et transparente.
    - Création d'une image du système d'exploitation initial pour l'architecture cible (ARM64), complet, configuré et ne requérant aucune connexion Internet, pouvant être flashée sur tout nœud physique.
    - Réalisation d'un Proof of Concept (PoC) fonctionnel sur des nœuds physiques à architecture ARM64 (spécifications similaires aux noeuds ANTS).
    - Réaliser des tests de résilience et développer la logique de récupération en cas de défaillance d'un ou plusieurs nœuds du cluster.
    - Investigation de méthodes ou recommandations pour sécuriser l'architecture du cluster et si le temps le permet, les mettre en place.
  ]
)